...
var favoriteBook = Book(title: "101 essays that will change the way you think", author: "Brianna Wiest", pages: 230, price: 141.12)

print("My favorite book is called \(favoriteBook.title), written by \(favoriteBook.author). It has \(favoriteBook.pages) pages and costs RSA\(favoriteBook.price)")


/*:
 Make a `Height` struct with two variable properties, `heightInInches` and `heightInCentimeters`. Both should be of type `Double`.
 
 Create two custom initializers. One initializer will take a `Double` argument that represents height in inches. The other initializer will take a `Double` argument that represents height in centimeters.
  ...
  It should then set the other property by calculating the right value from the passed in value.
...
/*:
 Now create a variable instance of `Height` called `someonesHeight`. 
 Use the initializer for inches to set the height to 65.
 ...
 */
 ...
 var myHeight = Height(heightInCentimeters: 162)
